# encoding: UTF-8
class ReporterCommentMailer < ActionMailer::Base
  default from: "#{I18n.t(:app_name)} <bairro.app@gmail.com>"

  def reporter_comment_email(report)
    @report = report
    name_and_mail_destination = "#{report.user.uuid}"
    small_description = truncate(report.description)
    mail(:to => name_and_mail_destination, :subject => "Novo comentário (#{report.municipality.name}) - #{small_description}")
  end
